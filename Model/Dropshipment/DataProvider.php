 ...
 */
namespace Adorncommerce\Dropshipment\Model\Dropshipment;
...
class DataProvider implements \Magento\Framework\View\Element\UiComponent\DataProvider\DataProviderInterface
{
    ...
    protected function searchResultToOutput(SearchResultInterface $searchResult)
    {
        ...
        $postcode = $this->dropshipmentFactory->create()->getCollection()->addFieldToFilter(
            'supplier_id',
            ['eq' => $supplierId]
        );
        foreach ($postcode->getItems() as $item) {
            $itemData = [];
            foreach ($item->getData() as $key => $attribute) {
                $itemData[$key] = $attribute;
            }
